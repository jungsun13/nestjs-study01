...
export class MenuService {
  ...
  async findBtnByParentId(parentId: string): Promise<ResultData> {
    const btnList = await this.menuRepo.find({
      where: { parentId },
  ...
  async deleteMenu(id: string): Promise<ResultData> {
    ...
    const { affected } = await this.menuManager.transaction(
      async (transactionalEntityManager) => {
        await transactionalEntityManager.delete(MenuPermEntity, { menuId: id });
        const result = await transactionalEntityManager.delete<MenuEntity>(
          MenuEntity,
          id,
        );
        return result;
  ...
  async updateMenu(dto: UpdateMenuDto): Promise<ResultData> {
    ...
    const { affected } = await this.menuManager.transaction(
      async (transactionalEntityManager) => {
        ...
        return await transactionalEntityManager.update<MenuEntity>(
          MenuEntity,
          dto.id,
          plainToInstance(MenuEntity, dto),
        );
